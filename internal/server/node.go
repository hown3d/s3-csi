...
func (n *NodeService) NodePublishVolume(ctx context.Context, req *csipb.NodePublishVolumeRequest) (*csipb.NodePublishVolumeResponse, error) {
    ...
    bucket, err := n.s3Client.GetBucket(ctx, name)
    if err != nil {
        var errNotFound *s3.ErrBucketNotFound
        if errors.As(err, &errNotFound) {
            return nil, status.Error(codes.NotFound, err.Error())
        }
        return nil, status.Error(codes.Internal, fmt.Sprintf("error getting s3 bucket: %s", err))
    }
